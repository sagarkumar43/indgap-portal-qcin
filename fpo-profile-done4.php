...
if (isset($_POST["Submit"])) {
    $Credit_Facilities_Availed_Bank = implode(",", $_POST["Credit_Facilities_Availed_Bank"]);
    $Credit_Facilities_Availed_FacilityType = implode(",", $_POST["Credit_Facilities_Availed_FacilityType"]);
    $Credit_Facilities_Availed_AmountReleasedRs = implode(",", $_POST["Credit_Facilities_Availed_AmountReleasedRs"]);
    $Credit_Facilities_Availed_AmountOutstandingRs = implode(",", $_POST["Credit_Facilities_Availed_AmountOutstandingRs"]);
    $Credit_Facilities_Availed_OperationStatusRegularIrregular = implode(",", $_POST["Credit_Facilities_Availed_OperationStatusRegularIrregular"]);
    $query1 = "UPDATE `fpo_registration_details` SET `Credit_Facilities_Availed_Bank`='$Credit_Facilities_Availed_Bank', `Credit_Facilities_Availed_FacilityType`='$Credit_Facilities_Availed_FacilityType', `Credit_Facilities_Availed_AmountReleasedRs`='$Credit_Facilities_Availed_AmountReleasedRs', `Credit_Facilities_Availed_AmountOutstandingRs`='$Credit_Facilities_Availed_AmountOutstandingRs', `Credit_Facilities_Availed_OperationStatusRegularIrregular`='$Credit_Facilities_Availed_OperationStatusRegularIrregular' WHERE FPO_Registration_ID  = '$_SESSION[FPO_Registration_ID]'";
    $exe_q = mysqli_query($db, $query1);
    if ($exe_q == TRUE) {
        header("Location:fpo-profile-done5.php");
    } else {
        header("Location:fpo-profile-done4.php?msg=fail");
    }
}
?>
<!DOCTYPE html>
   ...
   <body style="background-image: url('images/fpo-bg2.jpg');background-repeat: repeat-y;background-position: center;">
      ...
      <style>
         ...
         <main id="main">
            ...
            <section>
               <h3>Credit Facilities Availed</h3>
               <form method="POST" action="">

                <?php
                $query2 = mysqli_query($db, "SELECT * from fpo_registration_details where FPO_Registration_ID  = '$_SESSION[FPO_Registration_ID]' AND Credit_Facilities_Availed_Bank != '' AND DeletedStatus='0'");
                $row2 = mysqli_num_rows($query2);
                if($row2 == '1'){
                $rows2 = mysqli_fetch_assoc($query2);
                $rows2_Credit_Facilities_Availed_Bank = $rows2['Credit_Facilities_Availed_Bank'];
                $array2 = explode(',', $rows2_Credit_Facilities_Availed_Bank);
                $array3 = explode(',', $rows2['Credit_Facilities_Availed_FacilityType']);
                $array4 = explode(',', $rows2['Credit_Facilities_Availed_AmountReleasedRs']);
                $array5 = explode(',', $rows2['Credit_Facilities_Availed_AmountOutstandingRs']);
                $array6 = explode(',', $rows2['Credit_Facilities_Availed_OperationStatusRegularIrregular']);
                $count = 1;
                foreach($array2 as $value2){
                ?>
                <div class="container" id="credit_facilities">
                  <div class="row" id="<?php if($count > 1){echo 'credit_facilities2';}else{echo 'credit_facilities1';}?>">
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Bank</label>
                        <input value="<?php echo $array2[$count-1];?>"
                           name="Credit_Facilities_Availed_Bank[]" type="text" placeholder="Bank" />
                     </div>
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Facility Type</label>
                        <input value="<?php echo $array3[$count-1];?>"
                           name="Credit_Facilities_Availed_FacilityType[]" type="text" placeholder="Facility Type" />
                     </div>
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Amount Released (Rs)</label>
                        <input value="<?php echo $array4[$count-1];?>"
                           name="Credit_Facilities_Availed_AmountReleasedRs[]" type="text" placeholder="Amount Released" />
                     </div>
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Amount Outstanding (Rs)</label>
                        <input value="<?php echo $array5[$count-1];?>"
                           name="Credit_Facilities_Availed_AmountOutstandingRs[]" type="text"
                           placeholder="Amount outstanding(in lakhs) " />
                     </div>
                     <div class="col-md-6 form-group mt-3">
                        <label class="mb-2" style="float: left;">Operation Status (Regular/Irregular)</label>
                        <input value="<?php echo $array6[$count-1];?>"
                           name="Credit_Facilities_Availed_OperationStatusRegularIrregular[]" type="text"
                           placeholder="Operation Status (Regular/Irregular )" />
                     </div>
                     <?php if($count == 1){?>
                     <div class="col-md-2 form-group mt-5">
                        <input type="button" class="btn btn-success" onclick="add_credit_facilities();" value="+" style="width: 40%;">
                     </div>
                     <?php }?>
                     <?php if($count > 1){?>
                     <div class="col-md-2 form-group mt-5">
                        <input type="button" class="btn btn-danger mt-3" value="-" onclick="delete_equipment('credit_facilities2')" style="width: 40%;">
                     </div>
                     <?php }?>
                  </div>
               </div>
                ...
                <div class="container" id="credit_facilities">
                  <div class="row" id="credit_facilities1">
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Bank</label>
                        <input value=""
                           name="Credit_Facilities_Availed_Bank[]" type="text" placeholder="Bank" />
                     ...
                     <div class="col-md-2 form-group mt-5">
                        <input type="button" class="btn btn-success" onclick="add_credit_facilities();" value="+"
                           style="width: 40%;">
                     </div>
                  </div>
               </div>
            ...
            <a href="fpo-profile-done3.php"><button type="button" class="button" style="border: none;">Previous</button></a>
