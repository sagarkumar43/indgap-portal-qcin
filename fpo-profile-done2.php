...
if (isset($_POST["Submit"])) {
    $Procesing_Equipment_Activity = implode(",", $_POST["Procesing_Equipment_Activity"]);
    $Procesing_Equipment_Operational_Status = implode(",", $_POST["Procesing_Equipment_Operational_Status"]);
    $Procesing_Equipment_ValueRsinLakhs = implode(",", $_POST["Procesing_Equipment_ValueRsinLakhs"]);
    $Procesing_Equipment_Discription = implode(",", $_POST["Procesing_Equipment_Discription"]);
    $query1 = "UPDATE `fpo_registration_details` SET `Procesing_Equipment_Activity`='$Procesing_Equipment_Activity', `Procesing_Equipment_Operational_Status`='$Procesing_Equipment_Operational_Status', `Procesing_Equipment_ValueRsinLakhs`='$Procesing_Equipment_ValueRsinLakhs', `Procesing_Equipment_Discription`='$Procesing_Equipment_Discription' WHERE FPO_Registration_ID  = '$_SESSION[FPO_Registration_ID]'";
    $exe_q = mysqli_query($db, $query1);
    if ($exe_q == TRUE) {
        header("Location:fpo-profile-done3.php");
    } else {
        header("Location:fpo-profile-done2.php?msg=fail");
    }
}
?>
<!DOCTYPE html>
   ...
   <body style="background-image: url('images/fpo-bg2.jpg');background-repeat: repeat-y;background-position: center;">
      ...
      <style>
         ...
         <main id="main">
            ...
            <section>
               <h3>Procesing Equipment</h3>
               <form method="POST" action="">

                <?php
                $query2 = mysqli_query($db, "SELECT * from fpo_registration_details where FPO_Registration_ID  = '$_SESSION[FPO_Registration_ID]' AND Procesing_Equipment_Activity != '' AND DeletedStatus='0'");
                $row2 = mysqli_num_rows($query2);
                if($row2 == '1'){
                $rows2 = mysqli_fetch_assoc($query2);
                $rows2_Procesing_Equipment_Activity = $rows2['Procesing_Equipment_Activity'];
                $array2 = explode(',', $rows2_Procesing_Equipment_Activity);
                $array3 = explode(',', $rows2['Procesing_Equipment_Operational_Status']);
                $array4 = explode(',', $rows2['Procesing_Equipment_ValueRsinLakhs']);
                $array5 = explode(',', $rows2['Procesing_Equipment_Discription']);
                $count = 1;
                foreach($array2 as $value2){
                ?>
                <div class="container" id="euipment">
                  <div class="row" id="<?php if($count > 1){echo 'euipmentid2';}else{echo 'euipmentid1';}?>">
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Activity</label>
                        <input value="<?php echo $array2[$count-1];?>"
                           name="Procesing_Equipment_Activity[]" type="text" placeholder="Activity" />
                     </div>
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Operational Status</label>
                        <input value="<?php echo $array3[$count-1];?>"
                           name="Procesing_Equipment_Operational_Status[]" type="text" placeholder="Operational Status" />
                     </div>
                     <div class="col-md-4 form-group mt-3">
                        <label class="mb-2" style="float: left;">Value Rs in Lakhs</label>
                        <input value="<?php echo $array4[$count-1];?>"
                           name="Procesing_Equipment_ValueRsinLakhs[]" type="text" placeholder="Value Rs in Lakhs" />
                     </div>
                     <div class="col-md-6 form-group mt-3">
                        <label class="mb-2" style="float: left;">Discription</label>
                        <input value="<?php echo $array5[$count-1];?>"
                           name="Procesing_Equipment_Discription[]" type="text" placeholder="Discription" />
                     </div>
                     <?php if($count == 1){?>
                     <div class="col-md-4 form-group mt-5">
                        <input type="button" class="btn btn-success" onclick="add_equipment();" value="+" style="width: 25%;">
                     </div>
                     <?php }?>
                     <?php if($count > 1){?>
                     <div class="col-md-4 form-group mt-5">
                        <input type="button" class="btn btn-danger mt-3" value="-" onclick="delete_equipment('euipmentid2')" style="width: 25%;">
                     </div>
                     <?php }?>
                  </div>
               </div>
                ...
                <div class="container" id="euipment">
                  <div class="row" id="euipmentid1">
                     ...
                     <div class="col-md-6 form-group mt-5">
                        <input type="button" class="btn btn-success" onclick="add_equipment();" value="+"
                           style="width: 25%;">
                     </div>
                  </div>
               </div>
            ...
            <a href="fpo-profile-done1.php"><button type="button" class="button" style="border: none;">Previous</button></a>
