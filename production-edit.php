...
if (!isset($_SERVER['HTTPS']) or $_SERVER['HTTPS'] == 'off') {
 ...
 $Production_ID = $_GET['id'];
  $query_edit = mysqli_query($db, "SELECT * from production where Production_ID = '$Production_ID' AND  FPO_Registration_ID = '$_SESSION[FPO_Registration_ID]' AND DeletedStatus='0'");
  ...
  if (isset($_POST["CollectionCenter"])) {
    ...
    if ($query) {
      header("Location:production-edit.php?id=$Production_ID&msg=success");
    } else {
      header("Location:production-edit.php?id=$Production_ID&msg=fail");
    }
  } 
  ?>
<!DOCTYPE html>
